...
function gerarSenhasParaAgendamentoEditado(data, quantidadePessoas, indexIgnorado) {
  const totalBase = agendamentos
    ...
    .filter((item) => item.indexOriginal !== indexIgnorado && item.data === data)
    .reduce((acc, item) => acc + (item.pessoas?.length || 1), 0);
...
async function alternarCRMEnviado(index) {
  ...
  const confirmado = await confirmarAcao(
    jaEnviado ? "Desmarcar como enviado ao CRM?" : "Marcar como enviado ao CRM?",
    juntarNomes(agendamento.pessoas),
    jaEnviado ? "Desmarcar" : "Marcar"
  );
  ...
  agendamentos[index] = {
    ...agendamento,
    crmEnviado: !jaEnviado,
    crmEnviadoEm: !jaEnviado ? agoraISO() : null
  };

  salvar();
  filtrarAgenda();
  mostrarToast(!jaEnviado ? "Marcado como enviado ao CRM." : "CRM desmarcado.", "ok");
}
...
function filtrarAgenda() {
  ...
  listaAgenda.innerHTML = listaDoDia.map((item) => {
    ...
    return `
      <div class="agenda-item ${item.crmEnviado ? 'crm-enviado' : ''}">
        <p>
...
if (generoTMKInput) {
  generoTMKInput.value = generoTMK;
  generoTMKInput.addEventListener("change", (e) => {
    generoTMK = e.target.value === "F" ? "F" : "M";
